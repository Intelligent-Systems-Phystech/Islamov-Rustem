...
%%%%%%%%%%%%%%%%%%%%					
				\begin{tabular}{c} {\sf MAX-NEWTON} \\ Algorithm~\ref{alg:maxnewton} \end{tabular} &  $r_{k+1}\leq c r_k^2$      &local & quadratic & \cmark &  \ref{th:maxnewton}		\\	 \hline				
%%%%%%%%%%%%%%%%%%%%			 				
...
%%%%%%%%%%%%%%%%%%%%			 
			 \multirow{2}{*}{ 	\begin{tabular}{c} {\sf NEWTON-LEARN} \\ Algorithm~\ref{alg:NL2} \end{tabular}  } 
...
%%%%%%%%%%%%%%%%%%%%	
 \multirow{4}{*}{ \begin{tabular}{c} {\sf CUBIC-NEWTON-LEARN} \\ Algorithm~\ref{alg:CNL} \end{tabular}  } 
...
\noindent Let $\mU^T \Lambda \mU$ be the eigenvalue decomposition of $\mH^k + \lambda \mI$. Then we can transform the above equality to 
$$
...
\noindent Since $x^{k+1} = x^k + s^k$, from Lemma 1 in \citep{PN2006-cubic}, we have 
\begin{eqnarray}
...
\noindent First, by Lemma 1 in \citep{PN2006-cubic}, we can obtain 
$$
\langle \nabla P(x^k), y-x^k \rangle + \frac{1}{2} \langle \nabla^2 P(x^k)(y-x^k), y-x^k \rangle \leq P(y) - P(x^k) + \frac{M}{6}\|y-x^k\|^3. 
$$

\noindent For $ \|\mH^k + \lambda \mI - \nabla^2 P(x^k)\| $, we have 
\begin{eqnarray*}
	\|\mH^k + \lambda \mI - \nabla^2 P(x^k)\| &=& \|\mH^k - \nabla^2 f(x^k)\| \\
...
\noindent Let $\sigma_k \eqdef \frac{c_{k+1}}{C_{k+1}}$ for $k\geq 0$. Then $1 - \sigma_k = \frac{C_k}{C_{k+1}}$. We also have $\sigma_k \leq 1$ since 
$$
\max_{\xi\geq 0} \frac{\xi^2}{C_0 + \frac{\xi^3}{3}} =1. 
$$

\noindent Choose $y = \sigma_k x^* + (1-\sigma_k)x^k$. Then by the convexity of $P$, we have 
\begin{eqnarray*}
...
\noindent Therefore, we arrive at 
\begin{eqnarray*}
...
\noindent Then in order to guarantee $P(x^k) - P(x^*) \leq \epsilon$, we only need to let 
$$
...
\noindent Combining the above inequality and Theorem~\ref{th:concubic}, we can obtain 
$$
...
\noindent The method is summarized as Algorithm~\ref{alg:maxnewton}.
...
\noindent We now show that like original Newton's method and {\sf NEWTON-STAR}, {\sf MAX-NEWTON} also converges locally quadratically.
